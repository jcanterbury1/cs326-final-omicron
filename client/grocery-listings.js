...
async function addListings() {
  const listings = await (await fetch("/getGroceryListings")).json();
...
function createListing(storeName, storeAddress, storePrice, storeLikes) {
  ...
  likes.innerHTML = storeLikes + " &#11014";
  ...
  likeButton.addEventListener("click", async () => {
    const s = await (await fetch("/likeListing", {
      ...
      body: JSON.stringify({
        ...
        id: (storeName + storeAddress).split(" ").join("").toLowerCase(),
        category: "grocery",
        name: storeName
      }),
      headers: {
  ...
  readReviewsButton.addEventListener("click", () => {
    ...
    location.href = "/grocery-read-reviews";
  });
  ...
  writeReviewButton.addEventListener("click", () => {
    window.sessionStorage.setItem("name", name.innerHTML);
    window.sessionStorage.setItem("address", address.innerHTML);
    location.href = "/grocery-write-review";
  });
}