...
async function addListings() {
  const listings = await (await fetch("http://localhost:8000/getGroceryListings")).json();
...
function createListing(storeName, storeAddress, storePrice, storeLikes, index) {
  ...
  readReviewsButton.addEventListener("click", async () => {
    ...
    location.href = "../Jason/grocery-read-reviews.html";
  });
  ...
  writeReviewButton.addEventListener("click", async () => {
    ...
    location.href = "../Nadia/grocery-write-review.html";
  });
}