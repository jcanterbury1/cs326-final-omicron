...
async function addListings() {
  const listings = await (await fetch("/getLaundromatListings")).json();
...
function createListing(laundromatName, laundromatAddress, laundromatPrice, laundromatLikes) {
  ...
  likeButton.addEventListener("click", async () => {
    const s = await (await fetch("/likeListing", {
      ...
      body: JSON.stringify({
        ...
        id: (laundromatName + laundromatAddress).split(" ").join("").toLowerCase(),
        category: "laundromat",
        name: laundromatName
      }),
      headers: {
  ...
  readReviewsButton.addEventListener("click", () => {
    ...
    location.href = "/laundromat-read-reviews";
  });
  ...
  writeReviewButton.addEventListener("click", () => {
    window.sessionStorage.setItem("name", name.innerHTML);
    window.sessionStorage.setItem("address", address.innerHTML);
    location.href = "/laundromat-write-review";
  });
}