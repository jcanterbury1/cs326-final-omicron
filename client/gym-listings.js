...
async function addListings() {
  const listings = await (await fetch("http://localhost:8000/getGymListings")).json();
...
function createListing(gymName, gymAddress, gymPrice, gymLikes, index) {
  ...
  readReviewsButton.addEventListener("click", async () => {
    ...
    location.href = "../Jason/gym-read-reviews.html";
  });
  ...
  writeReviewButton.addEventListener("click", async () => {
    ...
    location.href = "../Nadia/gym-write-review.html";
  });
}